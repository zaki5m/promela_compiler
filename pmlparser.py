...
def p_sequence(p):
    ...
    if len(p) == 2:
        ...
        p[0] = p[1] + p[2]
def p_optionseq(p):
    """optionseq    : guard ARROW step
                    | guard ARROW step2 steps"""
    if len(p) == 4:
        p[0] = [("optionseq", [p[1], p[3]])]
    else:
...
def p_varrefs(p):
    """varrefs  : COMMA varref2
                | COMMA varref2 varrefs"""
    if len(p) == 3:
        p[0] = p[2]
    else:
        p[0] = p[2] + p[3]
...
def p_ivar(p):
    ...
    if len(p) == 2:
        ...
        p[0] = [("ivar", [p[1], p[3], p[5], p[6]])]

def p_ch_init(p):
    """ch_init  : LBRACKET const RBRACKET OF LBRACE typename RBRACE
                | LBRACKET const RBRACKET OF LBRACE typename2 typenames RBRACE"""
    if len(p) == 8:
        p[0] = ("ch_init", [p[2], p[6]])
    else:
...
def p_typenames(p):
    """typenames    : COMMA typename2
                    | COMMA typename2 typenames"""
    if len(p) == 3:
        p[0] = p[2]
    else:
        p[0] = p[2] + p[3]
...
def p_anyexprs(p):
    """any_exprs    : COMMA any_expr2
                    | COMMA any_expr2 any_exprs"""
    if len(p) == 3:
        p[0] = p[2]
    else:
        p[0] = p[2] + p[3]
...
def p_recv_argss(p):
    """recv_argss   : COMMA recv_arg2
                    | COMMA recv_arg2 recv_argss"""
    if len(p) == 3:
        p[0] = p[2]
    else:
        p[0] = p[2] + p[3]
...
def p_any_expr2(p):
    """any_expr2 : LPAREN any_expr RPAREN
                | any_expr binarop any_expr
                | unarop any_expr
                | LPAREN any_expr ARROW any_expr COLON any_expr RPAREN
                | LEN LPAREN varref RPAREN
